...
import { listByOrganizationId } from "../../services/classifybysize";
import UpdateButton from "../../components/buttons/UpdateButton";
import AddButton from "../../components/buttons/AddButton";
import { Box } from "@mui/material";
import GroupIcon from "@mui/icons-material/Group";

export default function SizeByOrganizationId() {
  ...
  const handleUpdate = (id: number) => {
    navigate(`/v1/sizedetail/${id}`);
  };

  const handleAdd = () => {
    navigate(`/v1/sizedetail/new?organization_id=${paramId}`);
  };
  ...
  return (
    <>
      <AppBarCustom title={`Size for Organization ID: ${paramId}`} />
      {loading ? (
        <Loading />
      ) : (
