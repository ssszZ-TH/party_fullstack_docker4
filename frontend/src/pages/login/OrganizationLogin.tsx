...
const OrganizationLogin: React.FC = () => {
  ...
  // ถ้า isAuthenticated เป็น true และ role เป็น organization_user จะ redirect ไปหน้า home
  useEffect(() => {
  ...
  // เรียก API organizationLogin และบันทึก token ลงใน Cookies
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { access_token } = await organizationLogin({ username, password });
      Cookies.set('access_token', access_token, { expires: 7, secure: true, sameSite: 'strict' });
      login(access_token, 'organization_user');
    } catch (err: any) {
