// ไฟล์นี้เป็น Modal สำหรับจัดการข้อมูลการจำแนกตามช่วงรายได้
...
// สอดคล้องกับ payload ที่ส่งไปยัง API endpoint `/v1/classifybyincome`
interface typeOfFormData {
  ...
  income_range_id: number | ""; // รหัสช่วงรายได้ (foreign key อ้างถึงตาราง incomerange)
}
...
interface typeofModalProps {
  ...
  incomeRangeDD: Array<typeOfDD>; // Dropdown สำหรับ IncomeRange
}

// Component หลักสำหรับแสดง Modal
export default function Modal_classifybyincome({
  open,
  onClose,
  initialDetail,
  onSubmit,
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      income_range_id: initialDetail.income_range_id === 0 ? "" : initialDetail.income_range_id,
    });
  }, [initialDetail]);
  ...
  const handleSubmit = () => {
    ...
    if (!formData.income_range_id) {
      alert("Income Range is required");
      return;
    }
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <Box sx={style}>
        ...
        {/* Dropdown สำหรับเลือก Income Range */}
        <FormControl fullWidth margin="normal">
          <InputLabel id="income-range-select-label">Income Range</InputLabel>
          <Select
            labelId="income-range-select-label"
          ...
          >
            <MenuItem value="">
              <em>Select Income Range</em>
            </MenuItem>
            {incomeRangeDD.map((item) => (
              <MenuItem key={item.id} value={item.id}>
                {item.text}
              </MenuItem>
            ))}
