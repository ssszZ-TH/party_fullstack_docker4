// ไฟล์นี้เป็น Modal สำหรับจัดการข้อมูลการจำแนกตามขนาดองค์กร (จำนวนพนักงาน)
...
// สอดคล้องกับ payload ที่ส่งไปยัง API endpoint `/v1/classifybysize`
interface typeOfFormData {
  ...
  employee_count_range_id: number | ""; // รหัสช่วงจำนวนพนักงาน (foreign key อ้างถึงตาราง employeecountrange)
}
...
interface typeofModalProps {
  ...
  employeeCountRangeDD: Array<typeOfDD>; // Dropdown สำหรับ EmployeeCountRange
}

// Component หลักสำหรับแสดง Modal
export default function Modal_classifybysize({
  open,
  onClose,
  initialDetail,
  onSubmit,
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      employee_count_range_id: initialDetail.employee_count_range_id === 0 ? "" : initialDetail.employee_count_range_id,
    });
  }, [initialDetail]);
  ...
  const handleSubmit = () => {
    ...
    if (!formData.employee_count_range_id) {
      alert("Employee Count Range is required");
      return;
    }
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <Box sx={style}>
        ...
        {/* Dropdown สำหรับเลือก Employee Count Range */}
        <FormControl fullWidth margin="normal">
          <InputLabel id="employee-count-range-select-label">Employee Count Range</InputLabel>
          <Select
            labelId="employee-count-range-select-label"
          ...
          >
            <MenuItem value="">
              <em>Select Employee Count Range</em>
            </MenuItem>
            {employeeCountRangeDD.map((item) => (
              <MenuItem key={item.id} value={item.id}>
                {item.text}
              </MenuItem>
            ))}
